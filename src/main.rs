...
fn count_file(path: &PathBuf) -> io::Result<Counts> {
    ...
    let bytes = n;
    // character count = UTF-8 character count
    let chars = String::from_utf8_lossy(&buf).chars().count() as u64;
...
fn main() -> io::Result<()> {
    let args = Args::parse();
    let show_all = !args.lines && !args.words && !args.bytes && !args.chars;
    ...
    if multiple {
        ...
        if sm {
            fields.push(format!("{:>8}", total.chars));
        }
